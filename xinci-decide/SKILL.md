...
2. **轻量决策书**,必含:词与任务;G1–G5 证据;窗口估计与 expiry(附推理);**跳过的闸门清单及"为何此刻无法执行"**(G6–G8 所需证据对天级新词客观不存在);48 小时发布计划(最小页面集);投入上限声明(损失封顶:一个域名 + 若干页面工时);风险确认(这是窗口赌注,不是被验证的生意);失效信号;升级通路说明(词若耐久,built 后可转回 tracking 走完整认定)。
...
- 提议与执行分离:转移经用户确认后才调 registrar;快道额外要求用户对"跳过闸门清单"逐条确认。
