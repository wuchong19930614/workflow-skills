...
2. `run_log.py --plan` 读恢复位置、上次词根和来源；conflict=true 时显式确定 run_id，不能新开运行覆盖旧任务。用户指定优先；新阶段采用 next_source/source_reason，恢复 started/blocked 阶段沿用原来源和 seed_value。读取 `discovery_feedback.py` 的方向统计与去重索引；root 从词根表中上次完成词根的下一个继续，lower_priority 方向默认选其他任务家族并记原因，跨轮不重置。旧清单只有文本时先核对，不猜历史进度。
...
5. 对照 duplicate_index 的主词、逐词表和原拒因，检查同一用户任务，避免同簇重复；换词根不视作新机会。`write_observation.py` 追加 scan，`ledger.py register` 登记 found。达到待核验池 5 个即可结束本批；源内无合格项如实记零。
6. `rank.py` 排序；用 `run_log.py --finish --run-id <ID> --outcome completed` 结束阶段，自动推导 next_step=verify，只补实际结果和本条新增计费。candidate-touched 包含本阶段登记的候选。报告注册/排除数、计费数及下一来源依据。
