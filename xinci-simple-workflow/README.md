...
核验前预登记核心/支撑任务清单；结算须交代每组，核心失败不能靠省略观察通过。日常用 `settle_candidate.py` 串起判定、收入、报告与校验；报告失败可原命令恢复。拒绝也验证对应门的最小证据。

运行恢复先找全部未完 ID；跳过 scan 仍明确落盘。阶段结束用 `run_log.py --finish` 自动推导轮次与下一步，减少重复填参。
...
报告额外披露最小产品、进入优势、成本与工时、收入爬坡、现金/含时间成本回本期和有预算的最小验证。缺失投入就明确未知，研究通过不等于商业验证。`investment.py plan` 冻结原始预测，`estimate` 只读比较假设，`feedback` 追加累计实际，`review` 对照预测并区分样本、预算和到期信号。反馈不自动修改资格或模型。
