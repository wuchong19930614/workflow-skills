...
完整核验前先登记用户卡点、解决方案、具体优势和交付依据。报告将研究结论与“值得小规模试做 / 需要补证据 / 暂不值得投入”分开，披露变现渠道依据、成本与工时、收入爬坡、回本期和最小验证计划。缺失投入明确为未知，研究通过不等于商业验证。
...
`discovery_feedback.py` 汇总来源收益、拒因与历史去重索引；样本足够时 --plan 优先选择有效来源，并保留探索轮。统计只使用可校验、可归因的学习样本，旧清单不猜测成本归属。`--replay` 只读比较准入口径；当前搜索量门槛和 $200 收入门槛保持不变。
