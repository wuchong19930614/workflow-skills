...
只读汇报，零判断，零写入：不调用 `ledger.py`，不改账本与任何产出；唯一例外是第 0 步经用户给出路径后的 `init_workspace.py`。账本是唯一事实来源，不从证据文件反推状态。
...
2. 如实转述：先各状态计数，再三段明细——待核验（按排序分）、已搁置（停留天数，超 90 天的行有提醒）、已验证（form / base / 报告路径，报告缺失会标出）。既有 verified 有 integrity_error 时明确标“待复核”，不能当作当前有效机会；只陈述事实，不加"建议尽快核验"之类的驱动性措辞；下一步做什么是用户的决定。
3. 用户要求或输出异常时补完整性检查：
   ```bash
