...
```
2. 如实转述输出:各状态候选数;默认仅非终态候选的年龄、距上次复查天数、expiry 余量;"expiry 已过且非终态"清单;可逆 G1/G2/G3 否决已到 `recheck_after` 的清单。先总数,后当前明细,异常置底单列。历史终态明细按需加 `--all`；`--json` 保留完整机器报告。暂缓到期只表示应复核证据是否变化，不强制形成结论。用户决定复核时,new 交 xinci-track,mature 交 xinci-mature;本 skill 不执行 reopen。
