...
只读汇报,零判断,零写入(唯一例外:第 0 步经用户给出路径后的数据区初始化)。本 skill 只陈述账本事实,不推荐动作、不安排日程、不执行任何状态转移。

面向用户的状态名称、时间字段和解释必须使用中文。机器内部状态码只用于账本与脚本，不得把 `captured`、`tracking`、`budget_reached` 等机器码直接作为自然语言报告；英文候选词、网址、文件名可原样保留。
...
- 零写入。本 skill 不调用 registrar,不改动账本与任何执行产出;唯一例外是第 0 步的数据区初始化(`init_workspace.py`,建目录与空账本,幂等,且必须在用户给出路径之后执行)。
...
- expiry 已过的候选照实列出,等用户处置;不代替用户提议 expired。处置归各阶段 skill,单步模式下由它们提议、用户确认,四条 expired 边各有归属:`tracking` 归 xinci-track 复查后提议;排队中的 `captured` 归 xinci-scan 开局接队时提议(以上两条在 `lane=mature` 上都归 xinci-mature,xinci-scan / xinci-track 不碰 mature);`screened` 与 `fast_grab_ready` 归 xinci-decide(前者是它快道模式的输入、后者是它的产出),由用户按本 skill 报出的到期清单把候选送给它。连续运行模式下四条一律归 xinci-run 运行循环步骤 1,那里是**标准授权直接转**,没有提议环节。无论哪种,都不是本 skill 的事。
