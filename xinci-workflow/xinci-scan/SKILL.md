...
按剩余深审容量分批推进：任务定义 → 低成本排除 → 共享市场与供给审计 → 注册分流。任务覆盖与暂定收入假设按公共证据契约写一次；各层只补新事实和门结论。
...
过了 G1 的候选按"离建站决策最近"排序,配额内做完整审计:G2 按闸门契约 G2;G3 按闸门契约 G3(先按 `g6_tentative_lines` 判占位否决是否生效,再按原始占位决策表审计;窗口期只用浏览器可得证据)。漏做深审入口预检的此刻补齐;六线全灭按第 2 层秒弃留痕,不占配额。暂定线未建立时不作 G3 结论,留在 `captured` 补证据。
