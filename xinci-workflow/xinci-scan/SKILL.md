...
**账本里的 `captured` 候选是上轮排队待深审的,本轮优先消化它们**(它们已过 G0/G4/G5/G1,直接进第 4 层)。
...
- **每轮 30–50 次为上限**:再多会挤爆上下文(每次首屏约 400–500 token),也会挤掉深审的空间。超出的候选连同未搜的一起排队(见第 4 层)。
...
**超出配额的候选不许丢弃**:注册成 `captured` 并带上已得的闸门结论,下轮第 0 层优先消化。
...
```json
"funnel": {
  "extracted": 22,            // 第 1 层提取的方向总数
  "rejected_zero_cost": 14,   // 第 2 层秒弃(已进淘汰索引)
  "rejected_g1": 4,           // 第 3 层 G1 否决(已注册并 rejected)
  "deep_audited": 3,          // 第 4 层实际深审
  "queued": 1                 // 超配额,注册为 captured 待下轮
}
```

**四个去向必须加总等于 extracted**——每个被提取的方向都要有归宿,不许无声丢弃。`validate_ledger.py` 强制此等式。
...
- **每个被提取的方向必须有归宿**:秒弃(进淘汰索引)、G1 否决(注册后 rejected)、深审(注册)、或超配额排队(注册为 captured)——四选一,**不许无声丢弃**。运行清单的 `funnel` 四项去向加总须等于 `extracted`,由 `validate_ledger.py` 强制。
