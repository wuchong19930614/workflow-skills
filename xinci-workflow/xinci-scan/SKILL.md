...
4. **先跑 G0(合法性与安全),再查陷阱类别(G5)。** G0 是零成本推理检查:违法、违反 ToS、欺诈供血、规避工具方向直接弃;命中 SERP 陷阱型的候选直接跳 G3 验证,失败即弃,不读 SERP 结构;命中前置筛除型(如纯品牌名)判定无任务即弃,不必跑 G3。
   **留痕分界**:G0 / 陷阱 / 无任务等零成本秒弃的方向,不注册,追加一行进淘汰方向索引(`- <日期> <词/方向> — <哪道门、为何>`);走到 G1 真实搜索及之后才被否决的,注册进账本再提议 rejected(SERP 证据落观察文件)。
5. **跑 G1,再 G2、G3、G4。** G1 一票否决且永不为省时间跳过。竞争判断读完整首页,禁止 top-3 定论;窗口期 G3 只用浏览器可得证据判否决线(footprint 实测属确认期,见闸门契约 G3 分层)。
...
9. **写运行清单** `运行/<日期>-xinci-scan.json`:打开过的来源、触及的候选、计费调用数(本阶段应为 0)。同日再次运行时文件名追加启动时刻(`<日期>-<HHMM>-xinci-scan.json`),不覆盖已有清单。
...
- 秒弃的方向必须留痕进淘汰方向索引,防止后续扫描重复评估;跑过 G1 的否决必须注册留痕。
