...
上图只画主干。省略的边有:`screened` 的快道 no-go → `rejected`;`captured` / `screened` / `tracking` / `fast_grab_ready` 的 expiry 过期 → `expired`;以及非终态 → `withdrawn` / `superseded`。完整状态图与每条边的提议人见[生命周期契约](xinci-core/生命周期契约.md)。
...
执行顺序 **G0 → G4 → G5 → G6/G7 零成本预筛(含 G6 深审入口预检) → G1 → G2 → G3**:零成本推理批先行,开浏览器的放后面。G6/G7 在扫描期不产生正式结论；G6 入口检查重复任务、官方计数口径与自助交付法律效力三类结构事实，并将实际影响映射到六条盈利线。只有全部适用线均暂定否决，或所有声称交付均依法无效时才在入口淘汰整候选；逐线暂定结论只供窗口期 G3 使用，正式 G6 仍在认定阶段产生。
