...
发现并验证英文 Google 搜索词,回答“这个词能不能撑起一个独立 SEO 站”:go 结论交付 md+html 建站决策书,no-go 只在账本登记决定性理由。

默认单步模式由用户逐次驱动,skill 不按时间字段自我调度;用户显式启动 `xinci-run` 后,它才在本次预算内连续编排各阶段。仓库层面的事(数据区配置、symlink 接入、测试)见[仓库根 README](../README.md)。

## 它做什么

从有日期的变化(法规、平台、厂商发布)与社区信号面提取搜索任务,逐个过九道闸门,给通过的候选做认定评分,最后形成建站 go/no-go 结论。go 交付决策书,no-go 只登记决定性理由。全流程留痕:账本记状态、证据文件记现场观察、索引记淘汰方向。
