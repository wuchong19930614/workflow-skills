---
name: xinci-run
description: 新词工作流的一体入口与连续运行驱动器:调用本 skill 即启动整个工作流,不间断循环"推进存量候选→扫描新候选→初筛→决策",直到找到合适建站的关键词(任一 go 决策)、Semrush 网页版额度实际耗尽,或预算用完(未指定时默认 max_rounds=6)才停。启动暗号 xinci_run:用户消息中出现该暗号即启动。也在用户说启动新词工作流、一直跑到找到为止、连续运行、run until found 时使用。单步操作用 xinci-scan/track/qualify/decide,看状态用 xinci-status。
---
...
1. **推进存量(优先;离 go 决策最近的先做)**:
   ...
   - **captured 候选(上轮扫描排队的)→ 按 `gates` 补跑缺的门**:gates 已到 G1 的直接做 G2/G3 深审;缺 G1 的(上轮超 G1 上限没搜)**先补 G1** 再进深审——排队身份不豁免任何闸门,G1 永不跳过。排队 expiry 已过的,提议 `captured→expired`,不占深审配额。带 `G3=veto_window_bet` 挂起等确认的**不动**(连续模式下推不动它,见硬规则),也不占配额。
     这是上轮欠的债,**必须在本轮扫描产生新债之前还**。**还债深审有自己的 ≤5 配额,与步骤 2 扫描的 ≤5 深审配额彼此独立**——还债不吃掉本轮新扫描的深审名额,否则扫出来的存活方向只能全部排队,积压反而更快。还债深审的次数记进本轮 `funnel.carryover_audited`(不参与加总等式,它不属于本轮 `extracted`)。
     **存量 captured 的消化归本步骤**:派子代理执行步骤 2 的扫描时,子代理从 xinci-scan 第 1 层开始,不再重跑它的第 0 层接队——两处都做会重复深审、双花配额。
   - **积压软闸(按存量压低扫描量,不停扫)**:步骤 1 做完后看 `captured` 存量(不含挂起等确认的),据此定本轮步骤 2 的提取目标:
     ...
     理由:漏斗每轮放行 10–20 个候选、深审只吃 5 个,不设闸的话队列单调增长,越积压越违背这条赛道的前提——晚三天发现的候选通常已经没价值。但**整轮停扫是过头的**:每轮进 10–20、出 ≤5,一旦积压就会连着好几轮不扫新,而默认预算只有 6 轮,很可能整次运行一个新方向都没扫到。压低提取量同样能让队列收敛(进得少、出得稳),又保证每轮都有新输入。只有当**本轮确实一条都没扫**(例如所有来源被拦)时,该轮 `funnel` 才写四项全 0——那是"本轮只推进存量"的记录,此时 `carryover_audited` 更要写实。
2. **扫描新候选**(提取目标按步骤 1 软闸表定,子代理从 xinci-scan 第 1 层开始):按 xinci-scan 流程(零成本批 G0→G4→G5 最先,G1 永不跳过;秒弃与 G1 否决都批量留痕进淘汰方向索引,账本只收还有下一步的候选)。每轮轮换来源与角度:信号面(HN、Product Hunt、即刻、应用商店、厂商博客……)与变化面(有日期的法规/平台/技术/成本变化)交替;被拦截的来源如实记录并换源。
3. **分流**(步骤 1 还债深审出的候选与步骤 2 扫描出的候选**一并分流**,别只分流新扫的):G0–G5 全过、窗口天级 → 立即走 xinci-decide 快道模式;窗口周/月级 → 转 tracking 入库;`G3=veto_window_bet` 的 → 留在 captured 挂起等用户单步确认(见硬规则)。然后继续循环。
4. **每轮收尾**:把本轮内容追加进本次运行的清单(来源、候选、计费调用数,以及 **`rounds[].funnel`**——本轮确实没扫就写四项全 0,那本身就是「本轮只推进存量」的记录;`validate_ledger.py` 自 2026-08-19 起强制每轮都有 funnel)。**还债深审的次数写进 `funnel.carryover_audited`**——它不参与加总等式(不属于本轮 `extracted`),但没有它,只还债的轮次看起来像什么都没干;去重命中的方向同理不计入 `extracted`。拒绝原因收敛时写 screen_unsatisfiable 假设报告——**然后继续运行**。
5. 回到步骤 1。
...
- **`G3=veto_window_bet` 不在标准授权内**(它是软化闸门结论,registrar 拒收 `by=xinci-run`)。深审判出这一档时,候选**留在 `captured`** 挂起:带 gates 与 expiry,观察文件记明豁免依据(免费实现清单 + 收录时差实测),清单 notes 记一句"待用户单步确认豁免",运行照常继续——它不是 blocker,后续轮次也不再动它。**不许转 `rejected`**(它没有失败的闸门,用户确认后本可以走快道),**不许换 `by` 硬推 `screened`**。
