---
name: xinci-run
description: 新词工作流的一体入口与连续运行驱动器:调用本 skill 即启动整个工作流,不间断循环"推进存量候选→扫描新候选→初筛→决策",直到产出任一 go 决策(全站 go,或一份标好价的快道赌注)、Semrush 网页版额度实际耗尽,或预算用完(未指定时默认 max_rounds=6)才停。启动暗号 xinci_run:用户消息中出现该暗号即启动。也在用户说启动新词工作流、一直跑到找到为止、连续运行、run until found 时使用。单步操作用 xinci-scan/track/qualify/decide,看状态用 xinci-status。
---
...
1. **推进存量(优先;离 go 决策最近的先做)**:
   ...
   - **captured 候选(上轮扫描排队的)→ 按 `gates` 补跑缺的门**:gates 已到 G1 的直接做 G2/G3 深审;缺 G1 的(上轮超 G1 上限没搜)**先补 G1** 再进深审——排队身份不豁免任何闸门,G1 永不跳过。排队 expiry 已过的,**即转** `captured→expired`(标准授权覆盖它,不必回头问用户),不占深审配额。带 `G3=veto_window_bet` 挂起等确认的**不动**(连续模式下推不动它,见硬规则),也不占配额。
